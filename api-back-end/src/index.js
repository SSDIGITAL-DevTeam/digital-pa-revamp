  ...
  express.static(path.join(__dirname, "/../upload"), {
    setHeaders: (res, filePath) => {
      ...
      if (filePath.endsWith(".pdf")) {
        ...
        res.setHeader("Content-Disposition", "inline"); // preview, bukan download

        // 🔑 Biar bisa embed di <iframe>
        res.setHeader("Cross-Origin-Resource-Policy", "cross-origin");
...
app.use(
  ...
  //     req.file = req.files.image[0]; // supaya kompatibel dengan code lama
  //   }
  //   if (req.files && req.files.pdf) {
  //     req.pdfFile = req.files.pdf[0]; // taruh di property terpisah
  //   }
  //   next();
  // },
  BlogController
...
// app.use("/api/v1/blog", upload.single("image"), upload.single("pdf"), BlogController);
