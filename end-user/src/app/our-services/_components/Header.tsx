...
export default function Header({ title, subtitle, className, subClassName }: Props) {
    return (
        <div className="flex flex-col gap-5 items-center lg:px-0 px-5">
            ...
            {subtitle && <p data-scroll data-scroll-speed="0.08" className={`w-full max-w-3xl text-center !leading-[150%] text-gray-800 text-base md:text-lg ${subClassName}`}>{subtitle}</p>}
        </div>
    )
}