        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {
                data.map((d: any, i:number) => (
                    <Link href={`/insights/${d.slug}`} key={`blogs-${i + 1}`} className="w-full  rounded-lg border-[1px] border-gray-300 shadow-md">
                        <div className="w-full relative">
                            <Image src={`${process.env.NEXT_PUBLIC_API_IMAGE_URL}/${d.image}`} alt={`image-${d.slug}`} width={1920} height={1080} className="md:object-cover md:object-top w-full md:h-[28vh]" quality={100} />
